...
namespace Catalyst.Windowing.SDL2
{
    ...
    internal static unsafe partial class SDL
    {
        ...
        public static int BlitScaled(Surface* src, Rect* srcrect, Surface* dst, Rect* dstrect) =>
            UpperBlitScaled(src, srcrect, dst, dstrect);

        public static int BlitScaled(PSurface src, Rect* srcrect, PSurface dst, Rect* dstrect) =>
            UpperBlitScaled(src, srcrect, dst, dstrect);

        public static int BlitSurface(Surface* src, Rect* srcrect, Surface* dst, Rect* dstrect) =>
            UpperBlit(src, srcrect, dst, dstrect);

        public static int BlitSurface(PSurface src, Rect* srcrect, PSurface dst, Rect* dstrect) =>
            UpperBlit(src, srcrect, dst, dstrect);

        public static int BlitScaled(Surface* src, ref Rect srcrect, Surface* dst, ref Rect dstrect) =>
            UpperBlitScaled(src, ref srcrect, dst, ref dstrect);

        public static int BlitScaled(PSurface src, ref Rect srcrect, PSurface dst, ref Rect dstrect) =>
            UpperBlitScaled(src, ref srcrect, dst, ref dstrect);

        public static int BlitSurface(Surface* src, ref Rect srcrect, Surface* dst, ref Rect dstrect) =>
            UpperBlit(src, ref srcrect, dst, ref dstrect);

        public static int BlitSurface(PSurface src, ref Rect srcrect, PSurface dst, ref Rect dstrect) =>
            UpperBlit(src, ref srcrect, dst, ref dstrect);

        [DllImport(LibraryName, EntryPoint = "SDL_ConvertPixels", CallingConvention = CallingConvention.Cdecl)]
        public static extern int ConvertPixels(int width, int height, uint src_format, void* src, int src_pitch,
            uint dst_format, void* dst, int dst_pitch);

        [DllImport(LibraryName, EntryPoint = "SDL_ConvertPixels", CallingConvention = CallingConvention.Cdecl)]
        public static extern int ConvertPixels(int width, int height, uint src_format, IntPtr src, int src_pitch,
            uint dst_format, IntPtr dst, int dst_pitch);
        ...
        public static extern Surface* CreateRGBSurface(uint flags, int width, int height, int depth, uint Rmask,
            uint Gmask, uint Bmask, uint Amask);

        public static void CreateRGBSurface(uint flags, int width, int height, int depth, uint Rmask, uint Gmask,
            uint Bmask, uint Amask, out Surface* surface)
        {
            surface = CreateRGBSurface(flags, width, height, depth, Rmask, Gmask, Bmask, Amask);
        }

        public static void CreateRGBSurface(uint flags, int width, int height, int depth, uint Rmask, uint Gmask,
            uint Bmask, uint Amask, out PSurface surface)
        {
            surface = CreateRGBSurface(flags, width, height, depth, Rmask, Gmask, Bmask, Amask);
        }

        [DllImport(LibraryName, EntryPoint = "SDL_CreateRGBSurfaceFrom", CallingConvention = CallingConvention.Cdecl)]
        public static extern Surface* CreateRGBSurfaceFrom(void* pixels, int width, int height, int depth, int pitch,
            uint Rmask, uint Gmask, uint Bmask, uint Amask);

        public static void CreateRGBSurfaceFrom(void* pixels, int width, int height, int depth, int pitch, uint Rmask,
            uint Gmask, uint Bmask, uint Amask, out Surface* surface)
        {
            surface = CreateRGBSurfaceFrom(pixels, width, height, depth, pitch, Rmask, Gmask, Bmask, Amask);
        }

        public static void CreateRGBSurfaceFrom(void* pixels, int width, int height, int depth, int pitch, uint Rmask,
            uint Gmask, uint Bmask, uint Amask, out PSurface surface)
        {
            surface = CreateRGBSurfaceFrom(pixels, width, height, depth, pitch, Rmask, Gmask, Bmask, Amask);
        }

        [DllImport(LibraryName, EntryPoint = "SDL_CreateRGBSurfaceFrom", CallingConvention = CallingConvention.Cdecl)]
        public static extern Surface* CreateRGBSurfaceFrom(IntPtr pixels, int width, int height, int depth, int pitch,
            uint Rmask, uint Gmask, uint Bmask, uint Amask);

        public static void CreateRGBSurfaceFrom(IntPtr pixels, int width, int height, int depth, int pitch, uint Rmask,
            uint Gmask, uint Bmask, uint Amask, out Surface* surface)
        {
            surface = CreateRGBSurfaceFrom(pixels, width, height, depth, pitch, Rmask, Gmask, Bmask, Amask);
        }
        ...
        public static extern Surface* CreateRGBSurfaceWithFormat(uint flags, int width, int height, int depth,
            uint format);

        public static void CreateRGBSurfaceWithFormat(uint flags, int width, int height, int depth, uint format,
            out Surface* surface)
        {
        ...
        [DllImport(LibraryName, EntryPoint = "SDL_CreateRGBSurfaceWithFormatFrom",
            CallingConvention = CallingConvention.Cdecl)]
        public static extern Surface* CreateRGBSurfaceWithFormatFrom(void* pixels, int width, int height, int depth,
            int pitch, uint format);

        public static void CreateRGBSurfaceWithFormatFrom(void* pixels, int width, int height, int depth, int pitch,
            uint format, out Surface* surface)
        {
            surface = CreateRGBSurfaceWithFormatFrom(pixels, width, height, depth, pitch, format);
        }

        public static void CreateRGBSurfaceWithFormatFrom(void* pixels, int width, int height, int depth, int pitch,
            uint format, out PSurface surface)
        {
            surface = CreateRGBSurfaceWithFormatFrom(pixels, width, height, depth, pitch, format);
        }
        ...
        [DllImport(LibraryName, EntryPoint = "SDL_CreateRGBSurfaceWithFormatFrom",
            CallingConvention = CallingConvention.Cdecl)]
        public static extern Surface* CreateRGBSurfaceWithFormatFrom(IntPtr pixels, int width, int height, int depth,
            int pitch, uint format);

        public static void CreateRGBSurfaceWithFormatFrom(IntPtr pixels, int width, int height, int depth, int pitch,
            uint format, out Surface* surface)
        {
            surface = CreateRGBSurfaceWithFormatFrom(pixels, width, height, depth, pitch, format);
        }
